...
;(function (root, factory) {
  ...
  ;(function ($) {
    ...
    $.expr.pseudos.icontains = function (obj, index, meta) {
      ...
      const haystack = ($obj.data('tokens') || $obj.text())
        .toString()
        .toUpperCase()
    ...
    Selectpicker.prototype = {
      ...
      createDropdown() {
        ...
        const inputGroup = this.$element.parent().hasClass('input-group')
          ? ' input-group-btn'
          : ''
        ...
        const drop =
          ...
          `</button>` +
          `<div class="dropdown-menu" role="combobox">${header}${searchbox}${actionsbox}<ul class="dropdown-menu inner" role="listbox" aria-expanded="false">` +
          `</ul>${donebutton}</div>` +
          `</div>`
          ...
          return (
            `<a tabindex="0"${
              ...
              that.options.liveSearchNormalize
                ? ` data-normalized-text="${normalizeToBase(
                    htmlEscape($(text).html())
                  )}"`
                : ''
            }${
              typeof tokens !== 'undefined' || tokens !== null
                ? ` data-tokens="${tokens}"`
                : ''
            } role="option">${text}<span class="${that.options.iconBase} ${
            ...
            if ($this.index() === 0) {
              ...
              const labelSubtext =
                typeof $parent.data('subtext') !== 'undefined'
                  ? `<small class="text-muted">${$parent.data(
                      'subtext'
                    )}</small>`
                  : ''
              const labelIcon = $parent.data('icon')
                ? `<span class="${that.options.iconBase} ${$parent.data(
                    'icon'
                  )}"></span> `
                : ''

              label = `${labelIcon}<span class="text">${htmlEscape(
                label
              )}${labelSubtext}</span>`
      ...
      render(updateLi) {
        ...
        if (
          ...
          if (
            ...
            const totalCount = $selectOptions.not(
      ...
      liHeight(refresh) {
        ...
        const header =
          this.options.header && this.$menu.find('.popover-title').length > 0
            ...
            : null
        const search = this.options.liveSearch
      ...
      setSize() {
        ...
        if (this.options.size === 'auto') {
          const getSize = function () {
            ...
            const lisVisible = Array.prototype.filter
              ? Array.prototype.filter.call(lis, hasClass('hidden', false))
              : that.$lis.not('.hidden')
            const optGroup = Array.prototype.filter
              ...
              if (maxOptions !== false || maxOptionsGrp !== false) {
                ...
                if (
                  ...
                  if (maxOptions && maxOptions == 1) {
                    ...
                    let maxTxtGrp = maxOptionsArr[1].replace(
                      '{n}',
                      maxOptionsGrp
                    )
      ...
      liveSearchListener() {
        ...
        this.$searchbox.on('input propertychange', function () {
          ...
          if (that.$searchbox.val()) {
            const $searchBase = that.$lis.not(
              '.is-hidden, .divider, .dropdown-header'
            )
            let $hideItems
            if (that.options.liveSearchNormalize) {
              $hideItems = $searchBase.not(
                `:a${that._searchStyle()}("${normalizeToBase(
                  that.$searchbox.val()
                )}")`
              )
            } else {
